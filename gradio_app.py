...
device = load_device()
keyword_generators = load_clip_generators(
    CLIP_MODELS, FEATURES_FILE_DIR, device, CACHE_DIR
)
...
def keywords(input_image):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        tasks = [
            executor.submit(generate_keywords, kg, input_image, TOP_K_LABELS)
            for kg in keyword_generators
        ]
...
with gr.Blocks() as demo:
    gr.Markdown(
        "Upload an image below to generate keywords using different CLIP model versions."
    )
    ...
    with gr.Row():
        for clip_model_name in CLIP_MODELS:
            output = gr.Label(
                label=f"{clip_model_name} Image Keywords", num_top_classes=TOP_K_LABELS
            )
